...
# Convert the Baseball Team Manager program from procedural to object-oriented.
# July 24 2020
...
def main():
    team = Baseball()
    data_driver = db()
    database = data_driver.load_from_file("players.csv")
    if (len(database) == 0):
        return
    ...
    while (True):
        ...
        if (choice == 1):
            ...
            data_driver.save_to_file("players.csv", lineup.get_players_db())
            print ("Bye!")
            break
        else:
            print ()
